...
void insertAfter(list *xs, item x) {
  ...
  new->x = x;
  new->next = xs->current->next;
  new->back = xs->current;

  xs->current->next->back = new;
  xs->current->next = new;
  xs->current = new;
}
...
void insertBefore(list *xs, item x) {
  ...
  new->x = x;
  new->next = xs->current;
  new->back = xs->current->back;

  xs->current->back->next = new;
  xs->current->back = new;
  xs->current = new;
}
...
bool deleteToAfter(list *xs) {
  if(none(xs)) return false;
  node *temp = xs->current->next;

  xs->current->back->next = temp;
...
bool deleteToBefore(list *xs) {
  if(none(xs)) return false;
  node *temp = xs->current->back;

  xs->current->back->next = xs->current->next;
  xs->current->next->back = temp;
